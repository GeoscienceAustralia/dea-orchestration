...
LOG = logging.getLogger("s3_sync_dates")
ARD = 'ARD'
...
def replace_metadata(granule, s3_bucket, s3_metadata_path):
    ...
    yaml_file = "{nci_path}/granule/ARD-METADATA.yaml".format(nci_path=NCI_DIR)
    ...
    # Create dataset ID based on the metadata path
    temp_metadata['id'] = str(uuid.uuid4(s3_metadata_path))

    # Attempt to write to S3 directly
